...
func (pt *PolygonType) unmarshalJSONOne(dec *arrowjson.Decoder) (PolygonValue, bool, error) {
	t, err := dec.Token()
	if err != nil {
	...
	for dec.More() {
		...
		if d, ok := ringTok.(arrowjson.Delim); !ok || d != '[' {
		...
		for dec.More() {
			...
			if d, ok := vertTok.(arrowjson.Delim); !ok || d != '[' {
